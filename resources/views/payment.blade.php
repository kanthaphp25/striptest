                            ...
                            @endforeach
                                <div class="row">
                                    <div class="col-sm-8">
										...
										<p><b>Product name</b> :   @if(!empty($productdetails['name'])) {{$productdetails['name']}} @else !some issue find @endif<p>
										<p><b>Price</b> :   @if(!empty($productdetails['price'])) {{$productdetails['price']}} @else !some issue find @endif<p>
                                        </div>
                                    </div>
                                </div>
                        </div>
            ...
            <article class="card">
                <div class="card-body p-5">
					...
					<ul class="nav bg-light nav-pills rounded nav-fill mb-3" role="tablist">
                        <li class="nav-item">
                            <a class="nav-link active" data-toggle="pill" href="#nav-tab-card">
                            <i class="fa fa-credit-card"></i> Enter card details</a>
                        </li>
                    </ul>
                    <div class="tab-content">
                        <div class="tab-pane fade show active" id="nav-tab-card">
                            ...
                            <form role="form" method="POST" id="paymentForm" action="{{ url('/payment')}}">
								...
								<input type="text" style="display:none" name="price" value="@if(!empty($productdetails['price'])) {{$productdetails['price']}} @else 0 @endif">
                                <div class="form-group">
